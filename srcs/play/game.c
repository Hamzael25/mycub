...
/*   Updated: 2023/12/01 23:12:59 by hamzaelouar      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3d.h"

int	init_game(t_win *win, t_data *data)
{
	win->mlx = mlx_init();
	if (!win->mlx)
		return (1);
	win->mlx_win = mlx_new_window(win->mlx, WINWIDTH, WINHEIGHT, "Cub3d");
	if (!win->mlx_win || init_texture(data, data->parse))
		return (1);
...
int	init_texture(t_data *d, t_parse *parse)
{
	...
	if (!d->we_text->img || !d->no_text->img || !d->so_text->img || !d->ea_text->img)
		return (1);
	d->no_text->addr = mlx_get_data_addr(d->no_text->img, \
		&d->no_text->bits_per_pixel, &d->no_text->line_length, &d->no_text->endian);
	d->so_text->addr = mlx_get_data_addr(d->so_text->img, \
		&d->so_text->bits_per_pixel, &d->so_text->line_length, &d->so_text->endian);
	d->ea_text->addr = mlx_get_data_addr(d->ea_text->img, \
		&d->ea_text->bits_per_pixel, &d->ea_text->line_length, &d->ea_text->endian);
	d->we_text->addr = mlx_get_data_addr(d->we_text->img, \
		&d->we_text->bits_per_pixel, &d->we_text->line_length, &d->we_text->endian);
	return (0);
}
